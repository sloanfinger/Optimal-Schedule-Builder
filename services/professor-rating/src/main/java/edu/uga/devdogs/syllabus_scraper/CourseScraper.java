...
public class CourseScraper {
  ...
  private static AspNetData getAspHiddenData(String html) {
    Matcher matcher = Pattern.compile(
      "id=\"__VIEWSTATE\" value=\"(.*?)\".*?id=\"__VIEWSTATEGENERATOR\" value=\"(.*?)\".*?id=\"__EVENTVALIDATION\" value=\"(.*?)\"",
      Pattern.DOTALL
    ).matcher(html);
   ...
   */
  public static void downloadAllSyllabusByInstructor(
    String department,
    ...
    try {
      ...
      SemesterInfo selectedSemester = new SemesterInfo(
        semesterCode,
        semesterName
      );
      ...
      response = Jsoup.connect(url)
        ...
        .data(
          "Button_ViewAll",
          "View+information+for+all+courses+with+Syllabus+files"
        )
        .data("ddlCourse", "-1")
        .method(Connection.Method.POST)
        .cookies(response.cookies())
      ...
      for (Integer ind : downloadInds) {
        ...
        if (
          contentDisposition == null || contentDisposition.isEmpty()
        ) continue;
        ...
        FileOutputStream out = new FileOutputStream(
          new java.io.File(String.format("%s/%s", path, fileName))
        );
   ...
   * @param path the path to where the file should be downloaded (local disk)
   */
  public static void downloadAllSyllabusByInstructorAndCourse(
    String department,
    String courseTitle,
    String instructorName,
    String path
    ...
    try {
      ...
      for (int i = 1; i < trTags.size(); i++) {
        ...
        if (
          info.getCourseTitle().equalsIgnoreCase(courseTitle) &&
   ...
   * @param path the path to where the file should be downloaded (local disk)
   */
  public static void downloadAllSyllabusByCourse(
    String department,
    String courseTitle,
    String path
  ) {
