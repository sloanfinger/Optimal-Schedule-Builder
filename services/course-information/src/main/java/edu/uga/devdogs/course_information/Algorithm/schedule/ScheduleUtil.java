...
public class ScheduleUtil {
  ...
  public static boolean validate(Schedule schedule) {
    ...
    for (TreeSet<Class> day : schedule.days().values()) {
      ...
      for (int i = 0; i < dayList.size() - 1; i++) {
        ...
        int currEndTime =
          currClass.endTime().getHour() * 60 + currClass.endTime().getMinute();
        int nextStartTime =
          nextClass.startTime().getHour() * 60 +
          nextClass.startTime().getMinute();
  ...
  public static double computeAverageProfessorQuality(Schedule schedule) {
    ...
    for (Section section : schedule.sections()) {
      ...
      if (
        section.professor().quality() == (Double) null ||
        section.professor().quality() == 0.0
      ) {
  ...
  public static void validateHard(Schedule schedule, HConstraints constraints) {
    if (schedule == null || constraints == null) {
      throw new IllegalArgumentException(
        "Schedule and/or constraints cannot be null"
      );
    }
  }
  ...
  public static double computeMaxDistance(Schedule schedule) {
    ...
    for (TreeSet<Class> day : schedule.days().values()) {
      ...
      for (int i = 0; i < dayList.size() - 1; i++) {
        ...
        double lat1 = courseInformationService.getLatitude(
          currClass.buildingNumber()
        );
        double lon1 = courseInformationService.getLongitude(
          currClass.buildingNumber()
        );
        double lon2 = courseInformationService.getLongitude(
          nextClass.buildingNumber()
        );
        double lat2 = courseInformationService.getLatitude(
          nextClass.buildingNumber()
        );
  ...
  public static double computeAverageIdleTime(Schedule schedule) {
    ...
    for (TreeSet<Class> day : schedule.days().values()) {
      ...
      for (Class _class : day) {
        int currStartTime =
          _class.startTime().getHour() * 60 + _class.startTime().getMinute();
        int currEndTime =
          _class.endTime().getHour() * 60 + _class.endTime().getMinute();
  ...
  public static double computeStartTime(Schedule schedule) {
    ...
    return (
      earliestStartTime.getHour() + (double) earliestStartTime.getMinute() / 60
    );
  }
  ...
  private static double computeEndTime(Schedule schedule) {
    ...
    return (
      latestStartTime.getHour() + (double) latestStartTime.getMinute() / 60
    );
  }
  ...
  public static double computeOverallObjective(Schedule schedule) {
    ...
    double normalizedProfessorQuality = normalizeValue(
      computeAverageProfessorQuality(schedule),
      professorQualityMinimum,
      professorQualityMaximum
    );
    double normalizedMaxDistance = normalizeValue(
      computeMaxDistance(schedule),
      maxDistanceMinimum,
      maxDistanceMaximum
    );
    double normalizedAverageIdleTime = normalizeValue(
      computeAverageIdleTime(schedule),
      averageIdleTimeMinimum,
      averageIdleTimeMaximum
    );
    ...
    return (
      normalizedProfessorQuality / 3 +
      (1 - normalizedMaxDistance) / 3 +
      (1 - normalizedAverageIdleTime) / 3
    );
  }

  public static double computeOverallObjectiveExtended(
    Schedule schedule,
    SConstraints softConstraints
  ) {
    // Checks if the parameters are valid
    if (schedule == null) {
      throw new IllegalArgumentException("Parameters cannot be null");
    ...
    if (softConstraints.gapDay() != null) {
      possSConstraintScore += computeGapDay(schedule, softConstraints.gapDay())
        ? 1
        : 0;
      possSConstraintsCount++;
    }
    if (softConstraints.prefStartTime() != null) {
      possSConstraintScore += normalizeValue(
    ...
    if (softConstraints.prefEndTime() != null) {
      possSConstraintScore +=
        1 -
        normalizeValue(
          computeEndTime(schedule),
          prefTimeMinimum,
          prefTimeMaximum
        );
      possSConstraintsCount++;
    ...
    possSConstraintScore = possSConstraintsCount == 0
      ? 0
      : possSConstraintScore / possSConstraintsCount;

    // computes the weighted sum. normalizedMaxDistance and normalizedAverageIdleTime are being subtracted from 1 since
    // the higher the value, the worse the schedule.
    double output =
      normalizedProfessorQuality / 4 + (1 - normalizedMaxDistance) / 4;
