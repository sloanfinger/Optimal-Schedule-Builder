...
public class BruteForceUtil {
   ...
   */
  public static Set<Course> dataPreSoftFilter(
    Set<Course> inputCourses,
    SConstraints currentConstraints
  ) throws Exception {
    ...
    for (Course course : inputCourses) {
      ...
      List<Section> validSections = getValidSections(
        currentConstraints,
        course
      );
   ...
   * @return a set of courses cleaned per the user's specification
   */
  public static Set<Course> dataPreHardFilter(
    Set<Course> inputCourses,
    HConstraints currentConstraints
  ) throws Exception {
    if (inputCourses == null || currentConstraints == null) {
      // Throws exception if parameter is null
      throw new IllegalArgumentException(
        "inputCourses and currentConstraints cannot be null"
      );
    }
    Set<Course> validCourses = new HashSet<>();
    for (Course course : inputCourses) {
      // Checks if course is in excluded courses and adds to return if isn't.
   ...
   * */
  private static List<Section> getValidSections(
    SConstraints currentConstraints,
    Course course
  ) {
    ...
    for (Section section : course.sections()) {
      ...
      for (Class c : section.classes()) {
        boolean isTooEarly = c
          .startTime()
          .isBefore(currentConstraints.prefStartTime());
        boolean isTooLate = c
          .startTime()
          .isAfter(currentConstraints.prefEndTime());
   ...
   */
  public static void ensureInitialCourses(
    Set<Course> inputCourses,
    Set<Schedule> validSchedules
  ) throws Exception {
