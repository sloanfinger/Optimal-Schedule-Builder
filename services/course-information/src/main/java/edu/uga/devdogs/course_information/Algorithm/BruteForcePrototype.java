...
public class BruteForcePrototype {
   ...
   */
  public static int[][] algorithmDriver(
    Set<Course> inputCourses,
    SConstraints softConstraints,
    HConstraints hardConstraints
  ) {
    Set<Course> outputCourses = new HashSet<>(inputCourses);

    try {
      outputCourses = BruteForceUtil.dataPreHardFilter(
        outputCourses,
        hardConstraints
      );
    } catch (Exception e) {
    ...
    try {
      outputCourses = BruteForceUtil.dataPreSoftFilter(
        outputCourses,
        softConstraints
      );
  ...
  public static int[][] optimize(
    ...
    for (Schedule schedule : validSchedules) {
      ...
      if (usesSoft) {
        overallObjective = ScheduleUtil.computeOverallObjectiveExtended(
          schedule,
          soft
        );
      } else {
    ...
    for (int i = 0; i < size; i++) {
      ...
      output[i] = ScheduleUtil.sectionsToInts(
        sortedSchedules.get(i).sections()
      );
    }
  ...
  public static Set<Schedule> generateValidSchedules(Set<Course> inputCourses) {
    ...
    generateValidSchedulesRecursive(
      sectionList,
      new ArrayList<>(inputCourses),
      validSchedules
    );
   ...
   * */
  private static void generateValidSchedulesRecursive(
    Set<Section> sections,
    List<Course> currCoursesToAdd,
