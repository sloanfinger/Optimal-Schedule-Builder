...
public class Pdf {
   ...
   * @return
   */
  public static List<Course2> parsePdf(
    String semester,
    ...
    try {
      downloadPDF(
        "https://apps.reg.uga.edu/soc/SOC" + semester + ".pdf",
        pdfLocation
      );
    } catch (Exception e) {
      e.printStackTrace();
    ...
    try {
      ...
      for (int i = 1; i <= numPages; i++) {
        ...
        if (i == 1) {
          //page one has extra useless lines
          topOffset = 4;
        }
        if (i == numPages) {
          //last page has extra useless lines
   ...
   */
  public static List<Course2> parsePage(
    String page,
    int topOffset,
    int bottomOffset
  ) {
    ...
    for (var elem : pageList) {
      ...
      if (isAllLetters(elem.substring(0, 4))) {
        //header is start with subject like csci. content will always start with a number, either crn or sec
        header = elem;
        continue; //set a header then continue to set its children
      }

      currContent = elem;
      if (currContent.substring(0, currContent.indexOf(" ")).length() < 5) {
        //in some cases the parser misses the sec column so take it from the previous entry
        currContent =
          prevContent.substring(0, prevContent.indexOf(".")) +
          currContent.substring(currContent.indexOf("."));
      }
  ...
  public static Course2 parseCourseString(String header, String content) {
    ...
    if (
      contentList.get(1).equalsIgnoreCase("A") ||
      contentList.get(1).equalsIgnoreCase("X")
    ) {
    ...
    int seatsAvailable = Integer.parseInt(
      contentList.get(contentList.size() - 1).trim()
    );
    ...
    for (var elem : contentList) {
      if (stateMeetingDays) {
        if (isAllLetters(elem)) {
          //meeting days can only be M, T, W, R, F
      ...
      if (stateBuildingRoomCampus) {
        //set the values for building, room, and campus
        if (count >= 3) {
      ...
      if (!(stateMeetingDays || stateMeetingTimes || stateBuildingRoomCampus)) {
        //the remaining of the string is professor name. sometimes the full name is given instead of only the last name
   ...
   */
  public static void downloadPDF(String uri, String fileLocation)
