...
public class CourseInformationApplication {
  ...
  @Bean
  @Order(1)
  CommandLineRunner buildingCommandLineRunner(
    BuildingRepository buildingRepository
  ) {
    return args -> {
      if (!updateBuildings) {
        System.out.println(
          "Skipping building data update as per configuration."
        );
        return;
      }
      ...
      try (InputStream inputStream = resource.getInputStream()) {
        ...
        CollectionType listType = objectMapper
          .getTypeFactory()
          .constructCollectionType(List.class, Building.class);
        ...
        System.err.println(
          "Failed to read or parse buildings.json: " + e.getMessage()
        );
      }
    };
  }
    ...
    return args -> {
      ...
      for (Course2 course : courses) {
        ...
        if (creditHoursStr.contains("-")) {
          try {
            creditHours = (int) Float.parseFloat(
              creditHoursStr.split("-")[0].trim()
            );
          } catch (NumberFormatException e) {
            System.err.println(
              "Invalid credit hours: " + course.getCreditHours()
            );
          }
        }
        ...
        if (professorLastName != null && !professorLastName.trim().isEmpty()) {
          ...
          if (professorIdCache.containsKey(professorLastName)) {
            ...
            existingProfessor = professorRepository
              .findById(cachedId)
              .orElse(null);
          } else {
            ...
            professorIdCache.put(
              professorLastName,
              newProfessor.getProfessorId()
            );
        ...
        Course existingCourse = courseRepository.findBySubjectAndCourseNumber(
          course.getSubject(),
        ...
        if (existingCourse == null) {
          ...
          try {
            description = scraper.getCourseDescription(
              course.getSubject(),
              course.getCourseNumber()
            );
          } catch (Exception e) {
            System.err.println(
              "Error fetching description for " +
                course.getSubject() +
                " " +
                course.getCourseNumber()
            );
        ...
        CourseSection courseSection = courseSectionRepository.findByCrn(
          course.getCrn()
        );
        ...
        // Lookup Building
        Building building = buildingRepository.findByBuildingCode(
          course.getBuilding()
        );
        if (building == null) {
  ...
  private LocalTime parseTime(String time) {
    try {
      if (
        time == null || time.isEmpty() || time.trim().equalsIgnoreCase("TBA")
      ) {
        return null;
      }
